...
smoothed_ocean = gaussian_filter(np.logical_not(is_land.values).astype(float),sigma=smooth_dist*pix_per_deg,
                                 mode=('reflect','wrap'))
...
for shortname in shortnames :
    ...
    coast = (smoothed_ocean>0.005)*basin
    ...
    coast.to_netcdf(f'/data/elegall/AR/ERA5/ocean_basins/{shortname}_coast.nc')
    ...
    if shortname in ['NPac','NAtl'] :
        ...
        for sub,subcoast in zip(['E','O'],[coastE,coastO]) : 
            ...
            subcoast.to_netcdf(f'/data/elegall/AR/ERA5/ocean_basins/{shortname}{sub}_coast.nc')
...
if __name__ == '__main__' :
    ...
    for ibasin,basin in enumerate(['NPac','SAtl','NAtl','SPac','IOce']) :
        print(basin)
        coast = xr.open_dataarray(f'/data/elegall/AR/ERA5/ocean_basins/{basin}_coast.nc')
    ...
    fig.savefig('/data/elegall/AR/ERA5/ocean_basins/map_of_oceanic_coasts.png')
# %%
